...
<body>
    ...
    {{View::make('layouts.header')}}
        <p><br></p>
        <h5 align="center">List of Cars</h5>
    ...
    <!-- Update Model -->
    <form action="" method="POST" class="car-update-record-model form-horizontal">
            ...
            <div class="modal-dialog modal-dialog-centered" style="width:55%;">
                <div class="modal-content" style="overflow: hidden;">
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" class="btn btn-success updateCar">Update
                        </button>
                    </div>
                </div>
            </div>
    ...
    <!-- Delete Model -->
    <form action="" method="POST" class="car-remove-record-model">
    ...
    <script>
        ...
        firebase.database().ref('Car/').on('value', function (snapshot) {
            ...
            $('#tbody').html(htmls);
            $("#submitCar").removeClass('desabled');
        });

        // Add Data
        $('#submitCar').on('click', function () {
            var values = $("#addCar").serializeArray();
            var id = values[0].value;
            var email = values[1].value;
            var carID = lastIndex + 1;
        ...
        $('body').on('click', '.updateData', function () {
            ...
            firebase.database().ref('Car/' + updateID).on('value', function (snapshot) {
                var values = snapshot.val();
                var updateData = '<div class="form-group">\
                    <label for="first_name" class="col-md-12 col-form-label">Car ID</label>\
                    <div class="col-md-12">\
        ...
        $('.updateCar').on('click', function () {
            var values = $(".car-update-record-model").serializeArray();
            var postData = {
                id: values[0].value,
        ...
        $("body").on('click', '.removeData', function () {
            ...
            $('body').find('.car-remove-record-model').append('<input name="id" type="hidden" value="' + id + '">');
        });

        $('.deleteRecord').on('click', function () {
            var values = $(".car-remove-record-model").serializeArray();
            var id = values[0].value;
            firebase.database().ref('Car/' + id).remove();
            $('body').find('.car-remove-record-model').find("input").remove();
